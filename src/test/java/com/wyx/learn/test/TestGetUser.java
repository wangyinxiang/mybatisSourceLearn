...
public class TestGetUser {
    ...
    public void testGetById() throws IOException {
        ...
        // sqlSessionFactory默认生成的SqlSession是DefaultSqlSession类型。selectOne()会调用selectList()
        User user = (User)session.selectOne("com.wyx.learn.mapper.UserMapper.getUser",1);
        UserMapper userMapper = session.getMapper(UserMapper.class);
        User user2 = userMapper.getUser(1);
        System.out.println(user.getDeptment() + " " + user.getUserName());
        System.out.println(user2.getDeptment() + " " + user2.getUserName());
    }
}
